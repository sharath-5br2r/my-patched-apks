...
class GooglePlaySession:
    ...
    def step5_get_app_details(self, package_name):
        ...
        if item.HasField("details") and item.details.HasField("appDetails"):
            ...
            if item.details.appDetails.versionString:
                version_string = item.details.appDetails.versionString
        ...
        print(f"[+] versionCode={version_code}, versionString={version_string}, offerType={offer_type}", file=sys.stderr)
        return version_code, version_string, offer_type
    ...
    def get_app_info(self, package_name):
        self.build_session()
        version_code, version_string, offer_type = self.step5_get_app_details(package_name)
        result = {
            "success": True,
...
def main():
    ...
    parser.add_argument("output_path", nargs="?", help="Output file path for the APK")
    parser.add_argument("--info", action="store_true", help="Only fetch and print app info, do not download")
    parser.add_argument("--dispenser", help="Dispenser URL (default: AuroraStore dispenser)")
    args = parser.parse_args()

    if not args.info and not args.output_path:
        parser.error("output_path is required unless --info is specified")

    try:
        session = GooglePlaySession(dispenser_url=args.dispenser)
        if args.info:
            session.get_app_info(args.package_name)
        else:
