...
yq -p toml -o toml -i ".youtube-morphe.version |= strenv(version) " configs/patches/morphe.toml
export version=$(get_experimental_version com.google.android.apps.youtube.music)
yq -p toml -o toml -i ".youtube-music-morphe.version |= strenv(version) " configs/patches/morphe.toml
